...
function isAuthorized(request, env) {
  const configuredToken = env.MAGAZINE_ADMIN_TOKEN || env.SUMMIT_ADMIN_TOKEN || env.ADMIN_TOKEN || '';
  if (!configuredToken) {
    return { ok: false, status: 500, error: 'Admin token is not configured. Add MAGAZINE_ADMIN_TOKEN in Cloudflare Pages environment variables.' };
  }
...
function publicFileUrl(request, key) {
  const url = new URL(request.url);
  url.pathname = '/api/magazine-admin-file';
  url.search = '';
...
export async function onRequestGet(context) {
  ...
  const url = new URL(request.url);
  const prefix = url.searchParams.get('prefix') || 'magazine/';
  const cursor = url.searchParams.get('cursor') || undefined;
  const limit = Math.min(Math.max(Number(url.searchParams.get('limit') || 500), 1), 1000);

  if (!prefix.startsWith('magazine/')) {
    return json({ success: false, error: 'Invalid prefix. Magazine admin can only list magazine/ objects.' }, 400);
  }
  ...
  try {
    ...
    return json({ success: false, error: error && error.message ? error.message : 'Failed to list Magazine uploads.' }, 500);
  }
}
