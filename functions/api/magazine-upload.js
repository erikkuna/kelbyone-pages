...
const ALLOWED_KINDS = new Set(['image', 'headshot']);
...
function sanitizeKind(kind) {
  const clean = (kind || 'image').toLowerCase().replace(/[^a-z0-9_-]/g, '');
  return ALLOWED_KINDS.has(clean) ? clean : 'image';
}
...
export async function onRequestPost(context) {
  ...
  const originalFilename = sanitizeFilename(url.searchParams.get('filename') || request.headers.get('x-file-name') || 'magazine-photo');
  const email = sanitizeEmail(url.searchParams.get('email') || request.headers.get('x-email') || 'unknown');
  const kind = sanitizeKind(url.searchParams.get('kind') || request.headers.get('x-upload-kind') || 'image');
  ...
  const key = `magazine/${date}/${email}/${kind}/${id}-${originalFilename}`;
  ...
  try {
    await env.GRID_EDIT_UPLOADS.put(key, request.body, {
      ...
      customMetadata: {
        ...
        source: 'kelbyone-magazine',
        kind
      }
    });

    const downloadUrl = `/api/magazine-file?key=${encodeURIComponent(key)}`;
    return json({
      success: true,
      key,
      downloadUrl,
