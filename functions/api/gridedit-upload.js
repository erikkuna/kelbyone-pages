const MAX_UPLOAD_BYTES = 100 * 1024 * 1024; // 100 MB initial cap; avoids Apps Script/base64 path entirely.
const ALLOWED_EXTENSIONS = new Set(['jpg','jpeg','png','tiff','tif','dng','cr3','cr2','nef','arw','raf','orf','rw2','heic','heif','webp']);
...
export async function onRequestPost(context) {
  ...
  const originalFilename = sanitizeFilename(url.searchParams.get('filename') || request.headers.get('x-file-name') || 'photo');
  ...
  const key = `gridedit/${date}/${email}/${id}-${originalFilename}`;
  ...
  try {
    ...
    const downloadUrl = `/api/gridedit-file?key=${encodeURIComponent(key)}`;
    return json({
      success: true,
      key,
      downloadUrl,
